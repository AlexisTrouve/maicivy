...
// Mock CV components
// DevPortrait = composant SERVEUR async (fetch loc+gitstats) → stub : ce test couvre le layout du CV,
// le rendu interne du Portrait est testé dans components/cv/__tests__/DevPortrait.test.tsx.
jest.mock('@/components/cv/DevPortrait', () => {
  return function MockDevPortrait() {
    return <div data-testid="dev-portrait">Dev Portrait Component</div>;
  };
});
...
describe('CVPage', () => {
  ...
  it('should render page heading', async () => {
    render(await CVPage({ params: { locale: 'fr' }, searchParams: {} }));

    expect(
      screen.getByRole('heading', { name: /Alexis - CV Dynamique/i })
    ).toBeInTheDocument();
  });
  ...
  it('should render with specified theme', async () => {
    render(await CVPage({ params: { locale: 'fr' }, searchParams: { theme: 'backend' } }));
  ...
  it('should render CVThemeSelector component', async () => {
    render(await CVPage({ params: { locale: 'fr' }, searchParams: { theme: 'frontend' } }));
  ...
  it('should render ExportPDFButton component', async () => {
    render(await CVPage({ params: { locale: 'fr' }, searchParams: { theme: 'devops' } }));
  ...
  it('should handle API error gracefully', async () => {
    ...
    await expect(CVPage({ params: { locale: 'fr' }, searchParams: {} })).rejects.toThrow(
  ...
  it('should have proper section IDs for navigation', async () => {
    const { container } = render(await CVPage({ params: { locale: 'fr' }, searchParams: {} }));
...
describe('CVPage - generateMetadata', () => {
  it('should generate metadata with default theme', async () => {
    const metadata = await generateMetadata({ params: { locale: 'fr' }, searchParams: {} });

    // themes.fullstack = "Full-Stack" dans messages/fr.json
    expect(metadata.title).toBe('CV Full-Stack - Alexis');
  ...
  it('should generate metadata with custom theme', async () => {
    ...
    // themes.backend = "Backend" dans messages/fr.json
    expect(metadata.title).toBe('CV Backend - Alexis');
    expect(metadata.description).toContain('Backend');
  });

  it('should capitalize theme in title', async () => {
    const metadata = await generateMetadata({
      params: { locale: 'fr' },
      searchParams: { theme: 'cpp' },
    });

    // themes.cpp = "C++" dans messages/fr.json
    expect(metadata.title).toBe('CV C++ - Alexis');
  });
  ...
  it('should include OpenGraph metadata', async () => {
    ...
    // themes.devops = "DevOps" dans messages/fr.json
    expect(metadata.openGraph?.title).toBe('CV DevOps - Alexis');
    expect(metadata.openGraph?.description).toContain('DevOps');
